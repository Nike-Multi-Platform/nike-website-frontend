...
import { getCategoriesByCatAndObject, getSubCategories } from '../../services/productServices';
...
const ListSubCategory = () => {
    ...
    const fetchSubCategories = async (category_id, productObjectId) => {
        try {
            const response = await getCategoriesByCatAndObject(category_id, productObjectId);
                                ...
                                {!collapsedCategories[category.categoryId] && (
                                    <ul>
                                        {category.subCategories.map((subCategory, subIndex) => (
                                            <li key={subIndex} className='text-lg text-gray-600 cursor-pointer' onClick={() => handleSubCategoryClick(subCategory.subCategoryId)}>
