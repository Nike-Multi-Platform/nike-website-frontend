...
const CategoriesProduct = () => {
  ...
  useEffect(() => {
    const subCategoryId = parseInt(
      searchParams.get("sub_categories_id") || "1",
      10
    );
    const updatedQuery = {
      ProductName: searchParams.get("searchText") || "-1",
      ProductObjectId: parseInt(
        searchParams.get("productObjectId") || "-1",
        10
      ),
  ...
  useEffect(() => {
    const fetchProductParents = async () => {
      try {
        const response = await productServices.getProductParents(
          objectQuery.SubCategoryId,
          objectQuery
        );
  ...
  return (
    <Layout
      className="max-w-[1400px] my-10 mx-auto"
    ...
    >
      ...
      <Layout className="bg-white" style={{ padding: "0 24px 24px" }}>
        <Content style={{ padding: 24, margin: 0, minHeight: 280 }}>
                ...
                <Menu onClick={({ key }) => handleSortChange(key)}>
                  {sortOptions.map((option) => (
                    <Menu.Item
                      ...
                      style={{
                        fontWeight:
                          (objectQuery.SortBy === "price" &&
                            option.value ===
                              (objectQuery.IsSortAscending
                                ? "price_asc"
                                : "price_desc")) ||
                          (objectQuery.SortBy === "createAt" &&
          ...
          <div className="pagination-container">
            <Pagination
              ...
              style={{ marginTop: 24 }}
              hideOnSinglePage={
                productParents?.length <= objectQuery.PageSize ? true : false
              }
            />
          </div>
        </Content>
